        ...
        onClick={() => {
          ...
          slides.forEach(slide => {
            ...
            if(slideDataID > (Math.floor(slides.length / 2)) + randomSlide) {
              slideCopy.setAttribute("data-id", (0 + (parseInt(slideDataID) - (slides.length + slideOffset))) + 20);
              slideCopy.style.transform = `translateX(${((((parseInt(slideDataID) + 20) - Math.floor(slides.length / 2)) + slides.length) * 100)}%)`;

              slideCopy.setAttribute("data-id", 20 + ((parseInt(slideDataID) - slides.length) - slideOffset));
              console.log(slideCopy.getAttribute("data-id"));
            } else if(slideDataID < randomSlide - (Math.floor(slides.length / 2))) {
              slideCopy.setAttribute("data-id", (parseInt(slideDataID) + (slides.length - slideOffset)) + 20);
              slideCopy.style.transform = `translateX(${((((parseInt(slideDataID) + 20) - Math.floor(slides.length / 2)) + slides.length) * 100)}%)`;

              slideCopy.setAttribute("data-id", 20 + ((parseInt(slideDataID) + slides.length) - slideOffset));
              console.log(slideCopy.getAttribute("data-id"));
            } else {
              slideCopy.setAttribute("data-id", (parseInt(slideDataID) - slideOffset) + 20);
              slideCopy.style.transform = `translateX(${((((parseInt(slideDataID) + 20) - Math.floor(slides.length / 2)) + slides.length) * 100)}%)`;

              slideCopy.setAttribute("data-id", 20 + (parseInt(slideDataID) - slideOffset));
              console.log(slideCopy.getAttribute("data-id"))
            }
          ...
          slideCopies.forEach(slide => {
            ...
            slide.style.transform = `translateX(${((slideDataID - 20) - Math.floor(slides.length / 2)) * 100}%)`;
